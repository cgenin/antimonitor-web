...
import {getProject, getVersionsForProject, groupByProjects, loadApis, loadProjects, loadTables} from './constants';
import {SET_APIS, SET_PROJECT, SET_PROJECTS, SET_TABLES, SET_VERSIONS} from './mutations-type';
import {MicroServiceState, Pagination, Project} from './types';
  ...
  [getProject]({commit}, id) {
    return projects()
      .then((l) => {
        ...
        const project: Project = l.find(p => p.id === id);
        commit(SET_PROJECT, project);
        return project;
      });
  },
  [getVersionsForProject]({commit}, id: string) {
